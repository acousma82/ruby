...
attr_reader :all_tasks
    ...
    def read_from_file(filename)
            ...
            status, *description = line.split(':') # here the (":") splits the string intop an array like [[], thing, to, do]. status, *description = usw assigns [] to status and "task" to an array of strings called description
            status = status.downcase.include?('x')                  # wenn include? string x dann setze den status auf true andernfalls ist er false
            add(Task.new(description.join.strip, status))    #alte .read methode:  IO.readlines(filename).each do |line| add(Task.new(line.chomp))
       end
     end
